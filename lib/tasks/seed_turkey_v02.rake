...
namespace :seed_turkey_v02 do
  ...
  task lesson_plan_translations: :environment do
    ...
    rec, status, message = Translation.find_or_update_translation(BaseRec::LOCALE_TR, LessonPlan.lp_table_header_key(true), "My Lesson Plans")
      throw "ERROR updating lp code translation: #{message}" if status == BaseRec::REC_ERROR
    ...
    rec, status, message = Translation.find_or_update_translation(BaseRec::LOCALE_EN, LessonPlan.lp_table_header_key(false), "Exemplar Lesson Plans")
      throw "ERROR updating lp code translation: #{message}" if status == BaseRec::REC_ERROR
    rec, status, message = Translation.find_or_update_translation(BaseRec::LOCALE_TR, LessonPlan.lp_table_header_key(false), "Exemplar Lesson Plans")
      throw "ERROR updating lp code translation: #{message}" if status == BaseRec::REC_ERROR
